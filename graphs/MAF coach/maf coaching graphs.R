...
caption <- "@Null_Hockey, data from war-on-ice.com"
title <- "Marc-Andre Fleury Goalie Coach Comparison"

maf <- read_csv("https://raw.githubusercontent.com/conorotompkins/AdjGSAA/master/combined/adjgsaa.combined.csv") %>%
  filter(name == "Marc-Andre.Fleury", toi > 2) %>%
  select(game_type, name, season, date, game_number, toi, adjgsaa60) %>%
...
ggplot(maf, aes(game_number, adjgsaa60)) +
  ...
  labs(y = "Adjusted GSAA per 60", x = "Game Number", title = title, caption = caption) +
...
ggplot(maf_coach_summary, aes(coach, adjgsaa60_mean, fill = coach)) +
  ...
  labs(y = "Mean AdjGSAA per 60", x = "Goalie Coach", title = title, caption = caption) +
  guides(fill = FALSE) +
  theme(plot.caption = element_text(size=12, hjust=1))
...
ggplot(maf_coach_summary, aes(coach, adjgsaa60_sd, fill = coach)) +
  ...
  labs(y = "Standard Deviation of Adjusted GSAA per 60", x = "Goalie Coach", title = title, caption = caption) +
  guides(fill = FALSE) +
  scale_fill_viridis(discrete = TRUE) +
  theme(plot.caption = element_text(size=12, hjust=1))
...
maf %>%
  ...
  ggplot(., aes(season, stddev, color = coach, group = 1)) +
    ...
    geom_point(size = 4) +
    ylim(c(0, 5)) +
    labs(x = "Season", y = "Standard Deviation", title = title, caption = caption) +
    scale_color_viridis(discrete = TRUE) +
    guides(color = guide_legend(title = "Goalie Coach")) +
    theme(plot.caption = element_text(size=12, hjust=1),
          plot.subtitle = element_text(size = 12),
          axis.text.x = element_text(size = 10))
ggsave("coach gssaa60 stddev line chart.png", width = 12, height = 6)
...
ggplot(maf, aes(adjgsaa60, fill = coach)) +  
  ...
  scale_y_continuous(expand = c(0, 0)) +
  scale_fill_viridis(discrete = TRUE) +
  labs(x = "Adjusted GSAA per 60", y = "Count of games", title = title, caption = caption) +
  guides(fill = FALSE) +
  theme(plot.caption = element_text(size=12, hjust=1),
        plot.subtitle = element_text(size = 12))
